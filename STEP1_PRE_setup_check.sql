spool STEP1_PRE_setup_check.log
...
BEGIN
    DBMS_OUTPUT.PUT_LINE('===== DATABASE CONFIGURATION PRECHECK =====');
    ...
    -- Database Role (FAIL if not PRIMARY)
    ...
    -- SPFILE (FAIL if not used)
    SELECT value INTO v_spfile
    FROM   v$parameter
    WHERE  name = 'spfile';

    IF v_spfile IS NOT NULL THEN
        v_status := 'PASS';
        print_line('SPFILE In Use', v_spfile, v_status);
    ELSE
        v_status := 'FAIL';
        print_line('SPFILE In Use', 'PFILE', v_status);
    END IF;

    -- ARCHIVELOG (FAIL if not ARCHIVELOG)
    ...
    -- LOG_ARCHIVE_DEST_1 (Informational)
    SELECT value INTO v_log_archive_dest1
    FROM   v$parameter
    WHERE  name='log_archive_dest_1';

    print_line('LOG_ARCHIVE_DEST_1', v_log_archive_dest1, 'OK');

    -- REMOTE_LOGIN_PASSWORDFILE (FAIL if not EXCLUSIVE)
    SELECT value INTO v_remote_pwdfile
    FROM   v$parameter
    WHERE  name='remote_login_passwordfile';

    IF v_remote_pwdfile = 'EXCLUSIVE' THEN
        v_status := 'PASS';
    ELSE
        v_status := 'FAIL';
    END IF;
    ...
    -- FRA (Informational)
    SELECT value INTO v_recovery_dest
    FROM   v$parameter
    WHERE  name='db_recovery_file_dest';

    SELECT value INTO v_recovery_size
    FROM   v$parameter
    ...
    -- OMF (Informational)
    SELECT value INTO v_db_create_dest
    FROM   v$parameter
